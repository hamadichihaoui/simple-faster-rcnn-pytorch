...
def preprocess_fix_size(img, min_size=600, max_size=1000):
    """Preprocess an image for feature extraction.
    ...
    # scale = min(scale1, scale2)
    img = img / 255.
    img = sktsf.resize(img, (C, min_size, max_size), mode='reflect', anti_aliasing=False)
    # both the longer and shorter should be less than
    # max_size and min_size
    if opt.caffe_pretrain:
        normalize = caffe_normalize
...
class Transform(object):
    ...
    def __call__(self, in_data):
        ...
        _, H, W = img.shape
        img = preprocess_fix_size(img, self.min_size, self.max_size)
        _, o_H, o_W = img.shape
        scale = o_H / H
